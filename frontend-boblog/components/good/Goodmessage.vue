<template>
  <div>
    ...
    <div v-if="count > 0" class="comment-container">
      <div class="comment-count">全部问答（{{ count }}）</div>
      <ul
      ...
      >
        ...
        >
          ...
          <div class="comment-item-detail">
            ...
            <div v-if="item.reply_info" class="comment-item-reply-content">
              <p
                v-for="replyItem in item.reply_info"
                :key="replyItem.id"
...
import { getMessagelist, createMessage } from '@/request/api/message'
import { isArray } from '@/lib/utils'
import { createMessageReply } from '@/request/api/messagereply'
...
export default {
  ...
  methods: {
    ...
    async submitReply(reply, index) {
      ...
      const [err] = await createMessageReply({
        good_id: this.$route.query.id,
        user_id: this.userId,
        email: this.commentEmail,
        message_id: this.commentId,
